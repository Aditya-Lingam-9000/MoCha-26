...
SUBMISSION_MODEL_CODE = """import json
from pathlib import Path
...
def main():
    print("Preparing DANN Submission Package...")
    
    # 1. Copy momask code to baseline bundle
    src_momask = CAREPD_DIR / "model" / "momask"
    dst_momask = BASELINE_DIR / "model" / "momask"
    if dst_momask.exists():
        shutil.rmtree(dst_momask)
    shutil.copytree(src_momask, dst_momask)
    
    (BASELINE_DIR / "model" / "__init__.py").touch()
    
    # 2. Copy MoMask Pre-trained Checkpoints
    src_weights = CAREPD_DIR / "assets" / "Pretrained_checkpoints" / "momask"
    dst_weights = BASELINE_DIR / "weights" / "momask"
    if dst_weights.exists():
        shutil.rmtree(dst_weights)
    shutil.copytree(src_weights, dst_weights)
    
    # 3. Copy DANN Model, Scalers, and PCA Tensors
    weights_dir = BASELINE_DIR / "weights"
    shutil.copy2(ROOT_DIR / "classifier_dann.pth", weights_dir / "classifier_dann.pth")
    shutil.copy2(ROOT_DIR / "scaler_mean.pt", weights_dir / "scaler_mean.pt")
    shutil.copy2(ROOT_DIR / "scaler_std.pt", weights_dir / "scaler_std.pt")
    shutil.copy2(ROOT_DIR / "pca_components.pt", weights_dir / "pca_components.pt")
    shutil.copy2(ROOT_DIR / "pca_mean.pt", weights_dir / "pca_mean.pt")
    
    # 4. Rewrite baseline_model.py
    baseline_py_path = BASELINE_DIR / "submission" / "baseline_model.py"
    with open(baseline_py_path, "w", encoding="utf-8") as f:
        f.write(SUBMISSION_MODEL_CODE)
        
    print("Code injected successfully. Zipping bundle for CodaBench...")
    
    # 5. Zip the bundle
    zip_path = ROOT_DIR / "submission.zip"
    
    exclude_dirs = [".git", "classifier"]
    exclude_files = ["classifier.pth", "classifier_fusion.pth", "fusion_scaler_mean.npy", "fusion_scaler_std.npy"]
    ...
    print(f"Done! DANN Submission package saved to {zip_path}")
